...
@section('title', 'تخصيص طلاب للباص - ' . $bus->number)
...
<div class="d-flex justify-content-between align-items-center w-100">
    <div>
        ...
        <p class="text-light mb-0 opacity-75">
            الباص: <strong class="text-info">{{ $bus->number }}</strong> | 
            المركز: <strong>{{ $bus->center->center_name ?? 'غير مرتبط بمركز' }}</strong> |
            المقاعد المتاحة: <strong class="text-success">{{ $available }}</strong>
        </p>
    </div>
...
<div class="row">
    ...
    <div class="col-lg-4">
        <div class="content-card mb-4 sticky-top" style="top: 100px;">
            <div class="text-center mb-4">
                ...
                <h4 class="text-primary">{{ $bus->number }}</h4>
                <p class="text-muted mb-0">{{ $bus->plate_number }}</p>
            </div>
            ...
            <div class="row text-center mb-4">
                <div class="col-4">
                    <h3 class="text-info mb-0">{{ $currentCount }}</h3>
                    <small class="text-muted">الحالي</small>
                </div>
                <div class="col-4">
                    <h3 class="text-success mb-0" id="availableCount">{{ $available }}</h3>
                    <small class="text-muted">المتاح</small>
                </div>
                <div class="col-4">
                    <h3 class="text-light mb-0">{{ $capacity }}</h3>
                    <small class="text-muted">الكلي</small>
                </div>
            </div>

            <div class="progress mb-3" style="height: 10px;">
                <div class="progress-bar bg-info" id="progressBar" style="width: {{ $percentage }}%"></div>
            </div>
            ...
            @if($bus->driver)
                <div class="driver-info mt-4 p-3 bg-dark rounded">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h6 class="mb-0 text-white">{{ $bus->driver->name }}</h6>
                            <small class="text-muted">{{ $bus->driver->mobile ?? $bus->driver->phone }}</small>
                        </div>
                    </div>
                </div>
            @endif
    ...
    <div class="col-lg-8">
        <div class="content-card">
            ...
            @if($students->count() > 0)
                <form id="assignForm" action="{{ route('admin.buses.store-students', $bus) }}" method="POST">
                    @csrf
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 p-2 bg-dark rounded">
                        <div class="form-check">
                            ...
                            <label class="form-check-label text-white" for="selectAll">
                                تحديد الكل (بحد أقصى {{ $available }})
                            </label>
                        </div>
                        <span class="badge bg-info" id="filteredCount">{{ $students->count() }} طالب/ة</span>
                    </div>

                    <div class="students-grid" id="studentsGrid">
                        @foreach($students as $student)
                            <div class="student-card" data-name="{{ $student->name }}" data-id="{{ $student->student_id }}">
                                <div class="form-check">
                                    <input type="checkbox" 
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                <div class="d-flex align-items-center mb-1">
                                                    <span class="student-gender-icon {{ $student->gender == 'ذكر' ? 'male' : 'female' }} me-2">
                                            ...
                                            <div class="text-end">
                                                <span class="badge {{ $student->preferred_schedule == 'صباحية' ? 'bg-warning' : 'bg-info' }}">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">
                        @if($bus->center_id)
                            جميع طلاب "{{ $bus->center->center_name ?? 'المركز' }}" تم تخصيصهم لباصات أخرى أو لا يوجد طلاب مقبولين.
                        @else
                    ...
                    <div class="d-flex justify-content-center gap-2 mt-4">
                        ...
                        <a href="{{ route('admin.students.index', ['center_id' => $bus->center_id, 'status' => 'approved']) }}" class="btn btn-info">
