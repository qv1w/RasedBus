    ...
    <div class="col-md-3 mb-3">
        <div class="stats-card">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h3 class="mb-0 text-white">{{ $stats['approved'] ?? 0 }}</h3>
    ...
    <div class="col-md-3 mb-3">
        <div class="stats-card">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h3 class="mb-0 text-white">{{ $stats['rejected'] ?? 0 }}</h3>
...
@if(in_array(session('admin_role'), ['developer', 'super_admin']))
...
اعاده تعيين التسجيل 
        ...
        <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteAllStudentsModal" {{ ($stats['total'] ?? 0) == 0 ? 'disabled' : '' }}>
            <i class="fas fa-trash-alt me-1"></i>
            حذف جميع الطلاب ({{ $stats['total'] ?? 0 }})
        </button>
    </div>
</div>
        ...
        <div class="modal-content bg-dark border-danger">
            ...
            <div class="modal-body">
                ...
                <ul class="text-danger">
                    <li><strong>{{ $stats['total'] ?? 0 }}</strong> طالب/ة نهائياً</li>
...
<div class="content-card mb-4">
    <div class="d-flex flex-wrap gap-2 align-items-center">
        ...
        <a href="{{ route('admin.students.index') }}" 
           class="btn btn-sm {{ !request('gender') && !request('type') && !request('status') ? 'btn-primary' : 'btn-outline-secondary' }}">
        ...
        <a href="{{ route('admin.students.index', array_merge(request()->except('gender'), ['gender' => 'أنثى'])) }}" 
           class="btn btn-sm {{ request('gender') == 'أنثى' ? 'btn-pink' : 'btn-outline-pink' }}">
            <i class="fas fa-female me-1"></i> بنات
        </a>
        <a href="{{ route('admin.students.index', array_merge(request()->except('gender'), ['gender' => 'ذكر'])) }}" 
           class="btn btn-sm {{ request('gender') == 'ذكر' ? 'btn-info' : 'btn-outline-info' }}">
        ...
        <a href="{{ route('admin.students.index', array_merge(request()->except('type'), ['type' => 'دار'])) }}" 
           class="btn btn-sm {{ request('type') == 'دار' ? 'btn-dar' : 'btn-outline-dar' }}">
            <i class="fas fa-mosque me-1"></i> دور التحفيظ
        </a>
        <a href="{{ route('admin.students.index', array_merge(request()->except('type'), ['type' => 'مركز'])) }}" 
           class="btn btn-sm {{ request('type') == 'مركز' ? 'btn-markaz' : 'btn-outline-markaz' }}">
            <i class="fas fa-graduation-cap me-1"></i> المركز
        </a>
        <a href="{{ route('admin.students.index', array_merge(request()->except('type'), ['type' => 'برنامج'])) }}" 
        ...
        <div class="col-md-2">
            ...
            <select name="schedule" class="form-select">
                ...
                @foreach($schedules as $key => $value)
                    <option value="{{ $key }}" {{ request('schedule') == $key ? 'selected' : '' }}>
                        {{ $value }}
                    </option>
                @endforeach
            </select>
        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="student-avatar {{ $student->gender == 'ذكر' ? 'male' : 'female' }} me-2">
                                <i class="fas fa-{{ $student->gender == 'ذكر' ? 'male' : 'female' }}"></i>
                            </div>
                            <div>
                    ...
                    <td>
                        <span class="badge {{ $student->preferred_schedule == 'صباحية' ? 'badge-period-morning' : 'badge-period-evening' }}">
                            <i class="fas fa-{{ $student->preferred_schedule == 'صباحية' ? 'sun' : 'moon' }} me-1"></i>
                            {{ $student->preferred_schedule }}
                        </span>
                    </td>
                    <td>
                        <a href="tel:{{ $student->mobile }}" class="text-info text-decoration-none">
                            {{ $student->mobile }}
                        </a>
                    </td>
                    <td>
