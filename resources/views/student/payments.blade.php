...
<head>
    ...
    <title>الدفعات - {{ $student->name }}</title>
                            ...
                            <div class="payment-item">
                                <div class="payment-header">
                                    ...
                                    <span class="payment-status status-{{ $statusClass }}">
                                        <i class="fas fa-{{ $statusClass == 'approved' ? 'check' : ($statusClass == 'pending' ? 'hourglass-half' : ($statusClass == 'awaiting' ? 'upload' : 'times')) }} me-1"></i>
                                        {{ $statusText }}
                                    </span>
                                </div>
                                ...
                                <div class="payment-details">
                                    ...
                                    <span><i class="fas fa-info-circle"></i>{{ $payment->description }}</span>
                                    @endif
                                    <span><i class="fas fa-calendar"></i>{{ $payment->created_at->format('Y/m/d') }}</span>
                                    @if($payment->due_date)
                                    <span><i class="fas fa-calendar-check"></i>الاستحقاق: {{ \Carbon\Carbon::parse($payment->due_date)->format('Y/m/d') }}</span>
                                    @endif
                                </div>
                                ...
                                <div class="upload-section">
                                    ...
                                    <div class="alert alert-custom alert-danger py-2 mb-3">
                                        <small><i class="fas fa-exclamation-triangle me-1"></i>سبب الرفض: {{ $payment->rejection_reason }}</small>
                                    </div>
                                    @endif
                                    
                                    {{-- تم إصلاح الـ URL ليستخدم URL مباشر بدلاً من route --}}
                                    <form action="{{ url('/student/payment/' . $payment->id . '/upload-receipt') }}" method="POST" enctype="multipart/form-data" class="upload-form">
                                        @csrf
